...
const OFFICE_ADDRESS = '366 Pearsall Avenue, Suite 2, Cedarhurst, NY 11516';
...
function legalFooterHtml() {
  ...
  return `<div style="margin-top:20px;padding-top:16px;border-top:1px solid #f1f5f9;font-size:10.5px;color:#aab4c0;line-height:1.6;">
    <p style="margin:0 0 8px;">This is an automated message sent from a no-reply mailbox. Please do not reply directly to this email.</p>
    ...
    <p style="margin:0 0 4px;">&copy; ${year} Premier Assist &nbsp;&middot;&nbsp; ${esc(OFFICE_PHONE)}</p>
    <p style="margin:0;">${esc(OFFICE_ADDRESS)}</p>
  </div>`;
}
...
export function mailer() {
  ...
  return nodemailer.createTransport({
    ...
    // Never let a stuck SMTP connection hang the whole request — bounded so the
    // submission can still return promptly even if the mail server is slow.
