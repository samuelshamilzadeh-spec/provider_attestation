...
// Where the notification goes. DISQUALIFIED_EMAIL, when set, overrides
// everything — one place to reroute these without touching the inboxes that
// receive scheduling and completion mail.
//   'yvy'      — office + Yeled V'Yalda (YVY referred the patient)
//   'standout' — office only (Yeled V'Yalda has no part in that flow)
export function disqualifyRecipients(flow) {
  if (process.env.DISQUALIFIED_EMAIL) return process.env.DISQUALIFIED_EMAIL;
  const list = flow === 'yvy'
    ? [process.env.OFFICE_EMAIL, process.env.YVY_EMAIL]
...
export async function notifyDisqualification({
  ...
  if (!to) throw new Error('No notification recipient is configured (set OFFICE_EMAIL or DISQUALIFIED_EMAIL)');
