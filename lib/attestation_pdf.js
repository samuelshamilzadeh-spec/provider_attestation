...
const TEMPLATE_PATH = path.join(process.cwd(), 'templates', 'attestation_template.pdf');

// ── Pre-populated values ─────────────────────────────────────────────────────
const DEFAULT_SCN_LEAD_ENTITY = 'HEALI';
...
// Keys here MUST match the exact `value` strings on the checkboxes in the
// frontend forms — they pass those strings unchanged.
...
// Generates the filled attestation PDF and returns its bytes (Uint8Array).
// payload: { memberName, dob (ISO), cin, criteria[], provider, date (ISO),
//            signatureDataUrl, scnLeadEntity? }
export async function generateAttestationPdf(payload) {
  const {
    memberName, dob, cin, criteria = [], provider, date, signatureDataUrl,
    scnLeadEntity
  } = payload;

  const leadEntity = (scnLeadEntity || '').trim() || DEFAULT_SCN_LEAD_ENTITY;
  ...
  // Page 1 — SCN header
  draw(page1, SCN_NAME_P1, leadEntity);

  // Page 2 — header + member info
  draw(page2, SCN_NAME_P2,  leadEntity);
  ...
  draw(page3, SCN_NAME_P3,    leadEntity);
