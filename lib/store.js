...
export function publicRecord(record, role) {
  ...
  // minus the raw file URLs. `disqualification` carries no files or tokens, so
  // it passes through as-is — the visit page uses it to show what was recorded
  // and by whom before offering to replace it.
  return {
    ...base,
    ...
    disqualification: record.disqualification || null
  };
}
