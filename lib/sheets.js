...
// Visit ID stays put as the internal matching key — findRowById() below reads
// from ID_COL, so its column must never move (moving it would orphan every
// existing row and create duplicates). New columns are appended AFTER it.
const HEADERS = [
  ...
  'Provider', 'Scheduled At', 'Visit ID', 'Link Opened At',
  'Not Qualified At', 'Not Qualified Reason'
];
const LAST_COL = 'Z'; // must match HEADERS.length (26 columns, A..Z)
...
function buildRow(record, origin) {
  ...
  const d = record.disqualification || {};
  const statusLabel = STATUS_LABELS[record.status] || 'Sent';
  ...
  return [
    ...
    // Whoever made the call — the clinician who signed, or the one who marked
    // the patient as not qualifying.
    c.provider || d.provider || '',
    s.submittedAt ? new Date(s.submittedAt).toLocaleString() : '',
    record.id,
    record.openedAt ? new Date(record.openedAt).toLocaleString() : '',
    d.at ? new Date(d.at).toLocaleString() : '',
    d.reasonLabel ? d.reasonLabel + (d.notes ? ` — ${d.notes}` : '') : ''
  ];
}
