...
// commonly mangles a multi-line PEM key: surrounding quotes get pasted in
// literally, or its newlines arrive as the two literal characters `\` + `n`
// instead of an actual line break. Undo both so a pasted-verbatim value works.
...
async function getAccessToken() {
  ...
  const key = normalizePrivateKey(process.env.GOOGLE_SHEETS_PRIVATE_KEY);
  if (!key.includes('BEGIN PRIVATE KEY')) {
    throw new Error('GOOGLE_SHEETS_PRIVATE_KEY does not look like a valid PEM key (no "BEGIN PRIVATE KEY" found) — check for stray quotes or a paste that dropped the newlines');
  }
  ...
  const signature = signer.sign(key, 'base64').replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
