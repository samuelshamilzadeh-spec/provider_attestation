...
const HEADERS = [
  'Completed At', 'Status', 'Appointment Date', 'Appointment Time', 'Patient Name',
  'Date of Birth', 'Phone', 'Email', 'Language', 'Lead Entity', 'Street Address',
  'City', 'State', 'ZIP', 'Insurance Carrier', 'Insurance Member ID',
  'Card Front', 'Card Back', 'Intake Source', 'Provider', 'Scheduled At', 'Visit ID'
];
const LAST_COL = 'V'; // must match HEADERS.length (22 columns, A..V)
const ID_COL = 'V';   // Visit ID's column — keep in sync with its position above
...
function buildRow(record, origin) {
  ...
  return [
    ...
    `${toTitleCase(record.firstName)} ${toTitleCase(record.lastName)}`.trim(),
